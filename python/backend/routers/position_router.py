...
position_router = APIRouter(prefix="/guild/{guild_id}/position", tags=["position"])
...
async def add_position_route(
    ...
    return await add_position_service(guild_id, dto, db, payload)
...
async def get_position_list_route(
    ...
    return await get_position_list_service(guild_id, db, payload)


@position_router.get("/{position_id}", response_model=PositionDTO)
async def get_position_detail_route(
    guild_id: int,
    position_id: int,
    db: AsyncSession = Depends(get_async_db),
    payload: Payload = Depends(verify_token),
):
    return await get_position_detail_service(guild_id, position_id, db, payload)
...
async def update_position_route(
    ...
    return await update_position_service(guild_id, position_id, dto, db, payload)
...
async def delete_position_route(
    ...
    return await delete_position_service(guild_id, position_id, db, payload)
