...
@preset_router.post("/{preset_id}/copy", response_model=PresetDTO)
async def copy_preset_route(
    guild_id: int,
    preset_id: int,
    dto: CopyPresetDTO,
    session: AsyncSession = Depends(get_session),
    ...
    return await copy_preset_service(guild_id, user_id, preset_id, dto, session)
