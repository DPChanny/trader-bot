...
# ─── Database (Crawler – MySQL) ─────────────────────────────────────────────


def get_crawler_db_port() -> int:
    return int(os.getenv("DB_PORT", "3306"))


def get_crawler_db_name() -> str:
    return os.getenv("DB_NAME", "trader_crawler")


def get_crawler_db_url() -> str:
    user = get_db_user()
    password = get_db_password()
    host = get_db_host()
    port = get_crawler_db_port()
    name = get_crawler_db_name()
